...
def save_test_split(
    ...
    """Save the held-out test split for future prediction and auditing."""
    ...
    test_labeled[TARGET_COLUMN] = test_labels

    test_features.to_excel(TEST_FEATURES_XLSX, index=False, engine="openpyxl")
    test_labeled.to_excel(TEST_LABELED_XLSX, index=False, engine="openpyxl")

    LOGGER.info("Saved feature-only test dataset to %s.", TEST_FEATURES_XLSX)
    LOGGER.info("Saved labeled test dataset to %s.", TEST_LABELED_XLSX)
...
def main() -> None:
    ...
    try:
        ...
        save_test_split(x_test, y_test, label_encoder)
